...
{
    [SerializeField] PlayerSnake playerSnake;
    ...
    private void Start()
    {
        playerSnake.resizeSnakeEvent.AddListener(UpdatePositions);
    ...
    {
        if (connectedBodyPart == null || playerSnake.StoneState.IsActive)
            return;
        float feetModifier = actuallyFeet ? 0.7f : 0.9f;
        float size = Mathf.Clamp(0.5f + playerSnake.SnakeList.Count * 0.015f, 0.5f, 1.05f);
        ...
        Quaternion rotation = actuallyFeet ? connectedBodyPart.transform.rotation : playerSnake.transform.rotation;
    ...
    public void UpdatePositions(int count)
    {
        ...
        if (activeState)
        {
            int index = actuallyFeet ? (int)((playerSnake.SnakeList.Count - 1) * 0.4f) : playerSnake.SnakeList.Count < 20 ? 1 : 2;
            if (connectedBodyPart != playerSnake.SnakeList[index])
            {
                SetBodyPart(playerSnake.SnakeList[index]);
            }
        }
    }
}
